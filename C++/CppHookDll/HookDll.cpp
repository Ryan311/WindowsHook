...
LRESULT CALLBACK ShellHookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	if (nCode == HSHELL_WINDOWCREATED)
	{
		...
		// wParam specifies the virtual key code
		// lParam specifies the key data
		::PostMessage(g_hWnd, WM_SHELLNOTIFY, wParam, lParam);
	}
	return ::CallNextHookEx(g_hHook, nCode, wParam, lParam);
}
...
BOOL WINAPI SetShellHook(BOOL bInstall, DWORD dwThreadId, HWND hWndCaller)
{
	BOOL bOk;
	g_hWnd = hWndCaller;
	...
	if (bInstall)
	{
		g_hHook = ::SetWindowsHookEx(WH_SHELL, ShellHookProc,
